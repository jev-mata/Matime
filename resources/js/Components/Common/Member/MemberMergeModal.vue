...
<template>
    <DialogModal closeable :show="show" @close="show = false">
        ...
        <template #content>
            <p>Merging the user <strong>{{ member.name }} </strong> into another one will transfer all time entries to
                the new user. <strong>This cannot be reverted!</strong></p>
            <div class="py-5 flex flex-col md:flex-row gap-6 items-center">
                ...
                <div class="flex-1">
                    <MemberCombobox v-model="newMember" placeholder="Select a member to merge into"></MemberCombobox>
                </div>
            </div>
        </template>
        <template #footer>

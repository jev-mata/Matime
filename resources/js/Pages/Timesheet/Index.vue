...
const isGroupedEmpty = computed(
  () => Object.keys(page.props.grouped as Record<string, Bimontly[]>).length === 0
);
const { addNotification } = useNotificationsStore();

const isUnsubmittedGroupedEmpty = computed(
  () =>
    Object.keys(page.props.unsubmitted_grouped as Record<string, Bimontly[]>).length === 0
);
const isArchiveGroupedEmpty = computed(
  () => Object.keys(page.props.archive_grouped as Record<string, Bimontly[]>).length === 0
);
...
async function approveRejectAll(type: "approve" | "reject") {
  ...
  try {
    ...
    addNotification("success", `${type === "approve" ? "Approved" : "Rejected"}`);
...
async function UnsubmittedRemindAll(type: "withdraw" | "remind") {
  ...
  try {
    const ids =
      type == "withdraw"
        ? page.props.archive_timesheets.map((t) => t.id)
        : page.props.unsubmitted_timesheets.map((t) => t.id);
      ...
      <div class="inline-block w-full align-middle">
        ...
        >
          ...
          <div
            ...
            class="flex border sticky top-0 dark:border-[#303F61] p-3"
          >
            <div class="flex-1">Name</div>
            <div class="flex-1">Groups</div>
            <div class="flex px-5">Hours</div>
          ...
          <template
            v-for="(userEntries, period) in page.props.unsubmitted_grouped"
            :key="period"
          >
            <TimesheetTable
              :get-period-info="getPeriodInfo"
        ...
        <div data-testid="client_table" class="grid w-full" v-if="activeTab == 'archive'">
          ...
          <template
            v-for="(userEntries, period) in page.props.archive_grouped"
            :key="period"
          >
            <TimesheetTable
              :get-period-info="getPeriodInfo"
